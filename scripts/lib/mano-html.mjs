...
export function analizarHTML(html) {
  ...
  return secciones.map(seccion => {
    const pila = [{tag:'raiz',fuera:false}], mano = [], pendientes = [], trazos = [], contenedores = [];
    ...
    for (const m of seccion.matchAll(re)) {
      ...
      if (m[1] != null) {
        if (!pila.at(-1).fuera && /class="con"/.test(m[1])) {
          try { const conexiones = JSON.parse(m[2]); if (conexiones.length) mano.push('conexiones'); conexiones.forEach(c => trazos.push(c.estilo === 'llave' ? 'llave' : 'flecha')); } catch (error) { throw new Error(`conexiones ilegibles al estimar capa a mano: ${error.message}`); }
        }
        continue;
      }
      if (!m[4]) continue;
      ...
      if (!fuera) {
        if (cls.some(c => ['nota','tabla'].includes(c))) contenedores.push(tag);
        if (cls.includes('sello')) trazos.push('sello');
        if (/\bdata-sub\b/.test(attrs)) trazos.push('subrayado');
        if (/\bdata-(?:tachar|tachon-img)\b/.test(attrs)) trazos.push('tachón');
        if (cls.includes('circ') || /\bdata-(?:circulo|circulo-img)\b/.test(attrs)) trazos.push('círculo');
      }
      if (!vacios.has(tag) && m[6] !== '/') pila.push({tag,fuera});
    }
    return { mano:mano.length, trazos:[...new Set(trazos)], contenedores:contenedores.length, pendientes:[...new Set(pendientes)] };
  });
}
