...
// `conSilueta`: además de la caja, arma una rejilla 16×16 de LA REGIÓN (no de una caja detectada, la región ya
// es fija en RAFAGAS) con qué celdas tienen tinta — misma idea que el glifo de tinta.mjs, para comparar FORMA
// (silueta), no solo presencia/posición, cuando dos lados comparten la misma región (el cursor de d_123).
export function medirRegion(rgba,w,h,roi,rojo=false,conSilueta=false) {
  const [rx,ry,rw,rh]=roi;let n=0,x0=w,y0=h,x1=-1,y1=-1;
  const y0px=Math.floor(ry*h),y1px=Math.min(h,Math.ceil((ry+rh)*h));
  const x0px=Math.floor(rx*w),x1px=Math.min(w,Math.ceil((rx+rw)*w));
  const silueta=conSilueta?new Array(256).fill(0):null;
  for(let y=y0px;y<y1px;y++)for(let x=x0px;x<x1px;x++){
    ...
    if(silueta){
      const xx=Math.min(15,Math.floor((x-x0px)/Math.max(1,x1px-x0px)*16));
      const yy=Math.min(15,Math.floor((y-y0px)/Math.max(1,y1px-y0px)*16));
      silueta[yy*16+xx]=1;
    }
  }
  return {pixeles:n,caja:n?{x:x0/w,y:y0/h,w:(x1-x0+1)/w,h:(y1-y0+1)/h}:null,...(conSilueta?{silueta}:{})};
}
// IoU de dos rejillas 16×16 (256 celdas 0/1): 1 = misma silueta, 0 = disjuntas, null si falta alguna.
export function iouSiluetas(a,b) {
  if(!Array.isArray(a)||!Array.isArray(b))return null;
  let inter=0,union=0;
  for(let i=0;i<256;i++){const x=Boolean(a[i]),y=Boolean(b[i]); if(x||y)union++; if(x&&y)inter++;}
  return union?+(inter/union).toFixed(3):null;
}
  ...
  for(const cfg of RAFAGAS){
    ...
    for(let f=cfg.corte;f<=cfg.fin;f++){
      ...
      const lados=await medir.evaluate(async ({urls,regiones})=>Promise.all(urls.map(async u=>{
        ...
        return Object.fromEntries(Object.entries(regiones).map(([k,r])=>[k,window.medirRegion(d,480,270,r,k==='movimiento'||k.startsWith('gris')?'neutro':k==='trazo'||k==='sello',k==='movimiento')]));
    ...
    casos.push({nombre:cfg.nombre,id:cfg.id,corte_frame:cfg.corte,fps:8,aparicion,estabilidad:estabilidadRegiones,
      // R21 (encargo): además de presencia y caja, se compara la FORMA (silueta 16×16 de la región) entre
      // referencia y réplica — no certifica identidad del cursor (dos formas distintas del mismo tamaño podrían
      // dar el mismo IoU), pero ya no se queda solo en «hay tinta en la caja».
      cursor:cfg.id==='r115'?{estado:'región compartida cursor/ruta medida en ambas; forma comparada por silueta 16×16; identidad exacta del asset requiere revisión visual',
        muestras:muestras.map(m=>({ms:m.ms,referencia:m.referencia.regiones.movimiento,replica:m.replica.regiones.movimiento,
          forma_iou:iouSiluetas(m.referencia.regiones.movimiento?.silueta,m.replica.regiones.movimiento?.silueta),dom:m.dom.cursor}))}:null,
