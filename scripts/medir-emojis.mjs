...
import { puntuarGlifo, pctRojo, pctApagado, svgAutonomo } from './lib/contraste-color.mjs';
...
await page.addScriptTag({ content: `window.puntuarGlifo = ${puntuarGlifo.toString()}; window.pctRojo = ${pctRojo.toString()}; window.pctApagado = ${pctApagado.toString()};` });
...
const medir = await page.evaluate(async ([items, fondos, apple, svgs]) => {
  ...
  const puntuar = datos => ({ ...Object.fromEntries(Object.entries(fondos).map(([nom, f]) => [nom, window.puntuarGlifo(datos, f)])), apagado: window.pctApagado(datos, .35) });
...
for (const set of ['apple', 'fluent']) for (const f of [...Object.keys(FONDOS), 'apagado']) {
...
tabla.metrica_apagado = '% del glifo a contraste ≥ 1.3:1 sobre blanco, opacidad efectiva .35';
...
console.log(`→ ${salida}`);
for (const set of ['apple', 'fluent']) for (const f of [...Object.keys(FONDOS), 'apagado']) console.log(`${set}/${f} < 20: ${bajos(set, f)}`);
